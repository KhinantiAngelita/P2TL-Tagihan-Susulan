...
@php
    ...
    $gabunganKeduanya = [
        ...
        ['label' => 'Paskabayar', 'nilai' => $totalGabunganPerGolongan['paskabayar']],
    ];
...
<div class="goltarif-card gabungan-card tone-gabungan" style="margin-bottom:20px;">
    <div class="goltarif-card-head">
        <div>
            ...
            <p class="sub" id="gabunganSub">Perbandingan Rp TS per golongan &mdash; Prabayar vs Paskabayar</p>
        </div>
        <div class="goltarif-head-actions">
            <span class="goltarif-year-badge">{{ $tahunAktif ?: '-' }}</span>
            <button type="button" class="copy-btn" id="gabunganCopyBtn" onclick="salinTabelGambar('capture-gabungan', this, document.getElementById('gabunganViewSelect').selectedOptions[0].text)">📷 Salin Gambar</button>
        </div>
    </div>
    ...
    <div id="capture-gabungan">
        ...
        <div id="view-gabungan-golongan">
            ...
            <p style="text-align:center;font-size:12px;color:#6b7690;margin:10px 0 20px;">
                ...
                <span style="display:inline-flex;align-items:center;gap:5px;"><span style="width:9px;height:9px;border-radius:50%;background:#e6a15a;display:inline-block;"></span>Cincin luar: Paskabayar</span>
            </p>
...
<div class="goltarif-grid">
    ...
    <div class="goltarif-card tone-paskabayar">
        <div class="goltarif-card-head">
                ...
                <h3>
                    ...
                    <div class="goltarif-title-select-wrap">
                        <select id="paskabayarViewSelect" class="goltarif-title-select" onchange="toggleGolTarifViewPaska(this.value)">
                            <option value="tarif">Gol Tarif Paskabayar</option>
                            <option value="daya">Gol per Daya Paskabayar</option>
                        </select>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6 6 6-6"/></svg>
                    </div>
                </h3>
            ...
            <div class="goltarif-head-actions">
                <span class="goltarif-year-badge">{{ $tahunAktif ?: '-' }}</span>
                <button type="button" class="copy-btn" id="paskabayarCopyBtn" onclick="salinTabelGambar('capture-paskabayar', this, document.getElementById('paskabayarViewSelect').selectedOptions[0].text)">📷 Salin Gambar</button>
            </div>
        </div>
...
function toggleGabunganView(view) {
    ...
    if (sub) {
        if (view === 'golongan') {
            sub.textContent = 'Perbandingan Rp TS per golongan — Prabayar vs Paskabayar';
        } else if (view === 'daya') {
            sub.textContent = 'Perbandingan Rp TS per daya — Prabayar vs Paskabayar';
        } else {
            sub.textContent = 'Total keseluruhan Rp TS — Prabayar vs Paskabayar';
        }
    }
}
