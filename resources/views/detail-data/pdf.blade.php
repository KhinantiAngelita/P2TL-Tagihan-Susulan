...
<body>
    ...
    <div class="detail-title">
        Data Detail <span class="detail-count">({{ $rows->count() }} baris)</span>
    </div>

    @php
        $rowChunks = $rows->chunk(150);
        $thead = '
            <thead>
                <tr>
                    <th>No</th><th>No Agenda</th><th>ULP</th><th>Tgl Agenda</th><th>IDPEL</th><th>Nama</th>
                    <th>Alamat</th><th>Gol</th><th>Tarif</th><th>Daya</th><th>KWH</th><th>Beban</th>
                    <th>KWH (Rp)</th><th>TS</th><th>Materai</th><th>Segel</th><th>Materia</th>
                    <th>RPPPJ</th><th>RPUJL</th><th>RPPPN</th><th>Tunai</th><th>Angsuran</th>
                    <th>Total</th><th>Tgl Register</th><th>No Register</th><th>Tgl SPH</th><th>No SPH</th>
                </tr>
            </thead>
        ';
    @endphp
    ...
    @foreach ($rowChunks as $chunkIndex => $chunk)
        ...
        <table class="data-table">
            ...
            @foreach ($chunk as $i => $row)
                <tr>
                    <td>{{ $row->no ?? (($chunkIndex * 150) + $i + 1) }}</td>
                    <td>{{ $row->no_agenda }}</td>
                    <td>{{ $row->ulp ?? '-' }}</td>
                    <td>{{ $row->tanggal_agenda?->format('d/m/Y') ?? '-' }}</td>
                    <td>{{ $row->idpel }}</td>
                    <td>{{ $row->nama }}</td>
                    <td>{{ $row->alamat }}</td>
                    <td>{{ $row->gol }}</td>
                    <td>{{ $row->tarif ?? '-' }}</td>
                    <td>{{ $row->daya ?? $row->daya_va ?? '-' }}</td>
                    <td>{{ number_format((float) $row->kwh, 0, ',', '.') }}</td>
                    <td>{{ number_format((float) $row->beban, 0, ',', '.') }}</td>
                    <td>{{ number_format((float) $row->kwh_rupiah, 0, ',', '.') }}</td>
                    <td>{{ number_format((float) $row->ts, 0, ',', '.') }}</td>
                    <td>{{ number_format((float) $row->materai, 0, ',', '.') }}</td>
                    <td>{{ number_format((float) $row->segel, 0, ',', '.') }}</td>
                    <td>{{ number_format((float) $row->materia, 0, ',', '.') }}</td>
                    <td>{{ number_format((float) $row->rpppj, 0, ',', '.') }}</td>
                    <td>{{ number_format((float) $row->rpujl, 0, ',', '.') }}</td>
                    <td>{{ number_format((float) $row->rpppn, 0, ',', '.') }}</td>
                    <td>{{ number_format((float) $row->tunai, 0, ',', '.') }}</td>
                    <td>{{ number_format((float) $row->angsuran, 0, ',', '.') }}</td>
                    <td>{{ number_format((float) $row->total, 0, ',', '.') }}</td>
                    <td>{{ $row->tanggal_register ? \Carbon\Carbon::parse($row->tanggal_register)->format('d/m/Y') : '-' }}</td>
                    <td>{{ $row->nomor_register }}</td>
                    <td>{{ $row->tanggal_sph ? \Carbon\Carbon::parse($row->tanggal_sph)->format('d/m/Y') : '-' }}</td>
                    <td>{{ $row->nomor_sph }}</td>
                </tr>
            @endforeach
            </tbody>
